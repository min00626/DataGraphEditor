...
namespace DataGraph
{
	public class InspectorView : VisualElement
	{
		...
		private DataGraphNode m_target;
		private DataGraphEditorNode m_targetEditorNode;
		...
		public void UpdateInspector(List<DataGraphEditorNode> selectedNodes)
		{
			if (selectedNodes.Count == 1)
			{
				m_target = selectedNodes[0].Node;
				m_targetEditorNode = selectedNodes[0];
			}
			else
			{
				m_scrollView.Clear();
			...
			{
				if (member.GetCustomAttribute<HideInInspectorView>() != null)
					continue;
				...
				if (member is FieldInfo field)
				{
					...
					if (section != null)
					{
						m_scrollView.Add(CreateSection(section.Label));
					}
				}
		...
		private void AddElementToList(IList list, MemberInfo member, object target)
		{
			...
			UpdateInspector(new List<DataGraphEditorNode> { m_targetEditorNode });
		}
